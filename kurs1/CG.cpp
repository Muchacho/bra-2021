...
void CG::Generation::head()
{
	...
	out << "includelib ../Debug/BRA_Lib.lib\n";
	...
	out << "_strcopy PROTO : BYTE, :BYTE\n";
	out << "_strcon PROTO : BYTE, :BYTE\n\n";
	out << "\n.stack 4096\n";
}
...
void CG::Generation::constants()
{
	...
	for (int i = 0; i < idTable.size; i++)
		if (idTable.table[i].idtype == IT::IDTYPE::L)
		{
			...
			if (idTable.table[i].iddatatype == IT::IDDATATYPE::SYM)
				out << " BYTE " << '\'' << idTable.table[i].value.vstr.str << '\'' << ", 0";
...
void CG::Generation::data()
{
	...
	for (int i = 0; i < idTable.size; i++)
	{
		if (idTable.table[i].idtype == IT::IDTYPE::V) {
			...
			if (idTable.table[i].iddatatype == IT::IDDATATYPE::STR)
				out << "; str\n";
			else if(idTable.table[i].iddatatype == IT::IDDATATYPE::SYM)
				out << "; sym\n";
			else
				out << "; int\n";
		}
		...
		case LEX_FUNCTION: {
			...
			while (lexTable.table[i].lexema != LEX_LEFTTHESIS)
			{
				if (lexTable.table[i].lexema == LEX_ID)
				{
					stackRet += 4;
					word += '_';
					word += idTable.table[lexTable.table[i].idxTI].scope;
					word += idTable.table[lexTable.table[i].idxTI].id;
					word += ": DWORD";
					out << word;
					word = "";
					//out << '_' << idTable.table[lexTable.table[i+1].idxTI].scope <<  idTable.table[lexTable.table[i].idxTI].id << ": DWORD";
		...
		case LEX_EQUAL: {
			...
			while (lexTable.table[i].lexema != LEX_SEMICOLON) {
				if (lexTable.table[i].lexema == LEX_ID) {
					...
					{
						if (!func) {
							out << "\tpush\t\t" << '_' << idTable.table[lexTable.table[i].idxTI].scope << idTable.table[lexTable.table[i].idxTI].id << "\n";
						}
						else {
							out << "\tpush\t\t" << '_' << idTable.table[lexTable.table[i].idxTI].scope << idTable.table[lexTable.table[i].idxTI].id << "\n";//1111
						}
				...
				if (lexTable.table[i].lexema == LEX_PROCENT)
				{
					out << "\n";
					out << "\tpop\t\tebx\n";
					out << "\tpop\t\teax\n";
